...
def get_search_results(keyword):
    response = requests.get(f"https://search.naver.com/search.naver?where=news&sm=tab_jum&query={keyword}&sort=1")
    html = response.text
...
def main():
    st.title("주식 퀴즈 생성기")
    keyword = st.text_input("보유 종목을 입력해주세요:")
    ...
    if keyword:
        with st.spinner('뉴스 데이터를 수집하는 중...'):
            ...
            completion_executor = CompletionExecutor(
                ...
                request_id='965b97ca-bf3d-4cee-a895-8e1204a53e46'
            )
            ############################################################
